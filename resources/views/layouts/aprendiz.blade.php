    ...
    <div class="flex min-h-screen flex-1 flex-col lg:h-screen lg:min-h-0">
        <header class="flex h-16 items-center justify-between border-b border-[#e3e7df] bg-white px-4 sm:px-5 lg:px-6 shadow-[0_8px_24px_rgba(0,0,0,0.03)]">
            ...
            <div class="flex items-center gap-3">
                ...
                @unless($enDashboard)
                    <a href="{{ route('aprendiz.dashboard') }}" title="Ir al panel principal"
                ...
                @endunless
                <h1 class="text-sm font-bold text-slate-900 sm:text-[15px]">@yield('titulo', 'Portal del Aprendiz')</h1>
            </div>

            <div class="flex items-center gap-3 sm:gap-4">
                {{-- Notificaciones dinámicas del rol (antes de la hora) --}}
                @include('layouts.notificaciones', ['rolPanel' => 'aprendiz'])
                ...
                <div class="relative" @keydown.escape.window="profileMenuOpen = false; profileModalOpen = false">
                    <button type="button"
                        ...
                        @else
                            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-green-100 text-sm font-bold text-[#39A900] ring-2 ring-white">
                                {{ auth()->user()->iniciales() }}
                            </div>
                        @endif
                        <div class="hidden text-right sm:block">
                            <p class="text-base font-bold text-slate-900">{{ auth()->user()->nombres ?? 'Aprendiz' }} {{ auth()->user()->apellidos ?? '' }}</p>
                            <p class="text-xs font-medium text-slate-500">Aprendiz SENA</p>
                        </div>
                    </button>
