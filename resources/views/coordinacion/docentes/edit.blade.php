...
<div class="mx-auto max-w-3xl space-y-6">
    ...
    <div class="rounded-2xl border border-red-200 bg-red-50/60 p-5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h3 class="text-sm font-bold text-red-700">Eliminar instructor</h3>
                <p class="mt-0.5 text-xs text-red-600/80">
                    Solo es posible si no tiene llamados, fichas ni liderazgos registrados; en ese caso usa «Inactivar».
                    Esta acción no se puede deshacer.
                </p>
            </div>
            <form method="POST" action="{{ route('coordinacion.docentes.destroy', $instructor->id_instructor) }}" class="shrink-0"
                  data-confirm="¿Eliminar definitivamente al instructor {{ $nombreCompleto }}? Esta acción no se puede deshacer."
