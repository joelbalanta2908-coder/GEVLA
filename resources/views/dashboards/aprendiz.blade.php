...
<div class="space-y-6">
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm font-medium text-gray-500">Estado Académico</p>
                    <p class="text-xl font-bold text-gray-900">{{ ucfirst($aprendiz->estado_academico ?? 'Activo') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm font-medium text-gray-500">Llamados Totales</p>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm font-medium text-gray-500">Procesos Disciplinarios</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $procesos->where('estado_proceso', 'activo')->count() }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="border-b border-gray-200 px-6 py-4">
            <h3 class="font-semibold text-gray-900">Mis últimos llamados de atención</h3>
        </div>
        ...
        @if($llamados->isEmpty())
            <div class="p-8 text-center text-gray-500">
                ...
                <p class="mt-1 text-xs text-gray-400">¡Sigue con tu buen desempeño académico y disciplinario!</p>
            </div>
        @else
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($llamados->take(5) as $llamado)
                            <tr class="hover:bg-gray-50/50">
                                ...
                                <td class="px-6 py-4">{{ $llamado->instructor->usuario->nombres ?? 'No asignado' }}</td>
                                <td class="px-6 py-4">
                                    @php
