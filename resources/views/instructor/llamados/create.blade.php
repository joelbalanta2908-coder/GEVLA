        ...
        <form method="POST" action="{{ route('instructor.llamados.store') }}" class="mt-6 space-y-5">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="tipo_llamado" required class="mt-1 w-full rounded-lg border-gray-300 text-sm focus:border-[#39A900] focus:ring-[#39A900]">
                        <option value="verbal" @selected(old('tipo_llamado') == 'verbal')>Verbal</option>
                        <option value="escrito" @selected(old('tipo_llamado') == 'escrito')>Escrito</option>
                    </select>
                </div>
            ...
            <div>
                ...
                <textarea name="descripcion_hechos" rows="4" required
                          class="mt-1 w-full rounded-lg border-gray-300 text-sm focus:border-[#39A900] focus:ring-[#39A900]">{{ old('descripcion_hechos') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="pruebas_aportadas" rows="3" placeholder="Ej: Control de asistencia firmado, pantallazos, etc."
                          class="mt-1 w-full rounded-lg border-gray-300 text-sm focus:border-[#39A900] focus:ring-[#39A900]">{{ old('pruebas_aportadas') }}</textarea>
            </div>
