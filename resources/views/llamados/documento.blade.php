...
<head>
    ...
    <style>
        body { font-family: 'Segoe UI', Calibri, Arial, sans-serif; color: #1e293b; margin: 34px; font-size: 13px; line-height: 1.6; }
        ...
        .encabezado { display: flex; border: 1px solid #1e293b; }
        .encabezado .logo { display: flex; align-items: center; justify-content: center; padding: 10px 18px; border-right: 1px solid #1e293b; }
        .encabezado .datos { padding: 8px 14px; font-size: 11px; line-height: 1.6; }
        .encabezado .datos .titulo { font-weight: 700; font-size: 12px; }
        .campo { margin: 14px 0 0; }
        .campo strong { text-transform: uppercase; }
        .motivo { margin-top: 22px; }
        .motivo .etiqueta { font-weight: 800; font-style: italic; text-decoration: underline; }
        .motivo p { margin: 8px 0; }
        .articulo { margin-top: 10px; }
        .articulo .lit { font-weight: 700; text-decoration: underline; }
        .aviso { margin-top: 18px; }
        .firmas { display: flex; justify-content: space-between; gap: 40px; margin-top: 56px; }
        .firma { flex: 1; text-align: left; }
        .firma .imagen { height: 70px; display: flex; align-items: flex-end; }
        .firma .imagen img { max-height: 68px; max-width: 230px; object-fit: contain; }
        .firma .linea { border-top: 2px solid #1e293b; margin-top: 4px; padding-top: 4px; font-weight: 600; }
        .firma .meta { font-size: 11px; color: #475569; }
        .pie { margin-top: 34px; font-size: 10px; color: #94a3b8; text-align: center; border-top: 1px solid #e2e8f0; padding-top: 8px; }
...
<body>
    ...
    @php
        ...
        $usuarioInstructor = $llamado->instructor?->usuario;
    ...
    <div class="encabezado">
        <div class="logo">
            @if($logoBase64)
                <img src="{{ $logoBase64 }}" alt="SENA" width="64">
            @else
                <strong>SENA</strong>
            @endif
        </div>
    ...
    <div class="firmas">
        @foreach([\App\Models\FirmaLlamado::ROL_INSTRUCTOR => 'Instructor', \App\Models\FirmaLlamado::ROL_APRENDIZ => 'Aprendiz'] as $rol => $etiqueta)
            ...
            <div class="firma">
                ...
                @else
                    <p class="meta">Pendiente de firma</p>
                @endif
            </div>
        @endforeach
    </div>
    ...
    @if($fc['registro'])
        <div class="firmas" style="margin-top: 42px;">
            <div class="firma" style="max-width: 46%;">
                <div class="imagen">
                    @if($fc['imagen'])
